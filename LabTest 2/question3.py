...
for a in range(num_of_quotes):
    ...
    # Validating username is only alphabetical and contains more than 2 letters
    first_name = input("What is your first name? ")
    while not first_name.isalpha() or len(first_name) <= 2:
        print("Invalid: Make sure its more then 2 letters and only consists of letters.")
        first_name = input("What is your first name? ")
    ...
    if user_choice == 1:
        ...
        bags_ordered += 1
        # Validating the amount of bags is greater then 0
        bags_amt = int(input("How many bags do you want (up to 4)? "))
        while bags_amt <= 0:
            print("Number has to be greater then 0")
            bags_amt = int(input("How many bags do you want (up to 4)? "))
        if bags_amt > 4:
            bags_count += 4
